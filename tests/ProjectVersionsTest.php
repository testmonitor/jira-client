...
class ProjectVersionsTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->projectVersion = ['id' => '1', 'key' => 'KEY', 'name' => 'ProjectVersion'];
    }
    ...
    public function it_should_return_a_list_of_project_versions()
    {
        ...
        $service->shouldReceive('request')
            ->once()
            ->andReturn(new Response(200, ['Content-Type' => 'application/json'], json_encode(['values' => [$this->projectVersion]])));

        // When
        $projectVersions = $jira->projectVersions('12345');

        // Then
        $this->assertIsArray($projectVersions);
        $this->assertCount(1, $projectVersions);
        $this->assertInstanceOf(ProjectVersion::class, $projectVersions[0]);
        $this->assertEquals($this->projectVersion['id'], $projectVersions[0]->id);
